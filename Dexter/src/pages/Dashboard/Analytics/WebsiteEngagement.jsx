    ...
    <div className="relative w-1/2 p-4 mb-4 bg-white border rounded-lg h-fit max-md:w-full">
      ...
      <div className="flex justify-evenly max-md:gap-12 mt-4 items-center   mx-auto  lg:flex-row max-md:flex-wrap p-4   rounded-lg bg-[#f5f7f9]">
        {engagementMetrics.map((metric, index) => (
          <div
            key={index}
            className="[&:not(:first-child)]:border-l border-l-[#d5d9e1] [&:not(:first-child)]:pl-4 max-md:[&:not(:first-child)]:border-none max-md:[&:not(:first-child)]:pl-0  [&:not(:last-child)]:mr-3"
          >
            <Metrics
              metric={metric}
            ...
            />
            <p className="text-[#9795fa] ml-10 text-[10px] -mt-1 font-semibold ">
              View {metric.toView}
            </p>
          </div>
        ))}
