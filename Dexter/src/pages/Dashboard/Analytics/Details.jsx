...
const metaData = {
  metaTitleData: {
    current: "5 Reasons Why Does Your Frenchie Pee on His Bed? - Best Dog Resources",
    recommendations: [
  ...
  metaDescriptionData: {
    current: "Does your French bulldog often pee on the bed even after taking them to the toilet and giving them treats as a reward? If you, like me, believe that dogs don't mess with where they sleep or eat, you are sadly mistaken. Like any other breed, Frenchies may exhibit certain behaviors that can be concerning.",
    recommendations: [
      ...
      <div className="lg:w-[23%] transition-all duration-300 absolute top-0 bottom-0 right-0 max-md:w-full max-md:top-12 md:w-[50%] bg-white p-4 overflow-y-scroll">
        ...
        <div className="flex flex-col gap-4 mt-4">
          <div className="bg-[#f0f0ff] rounded-lg p-4">
            ...
            <p className="font-semibold">Current: <span className="text-[#6d68fb]">{analytics?.metaTitleData?.current}</span></p>
          ...
          <div className="bg-[#f0f0ff] rounded-lg p-4">
            ...
            <p className="font-semibold">Current: <span className="text-[#6d68fb]">{analytics?.metaDescriptionData?.current}</span></p>
            <h4 className="font-bold mt-2">Recommendations:</h4>
            <ul className="bg-[#e0e0ff] p-2 rounded-lg">
              {analytics?.metaDescriptionData?.recommendations.map((rec, index) => (
                <li key={index} className="text-sm">{rec}</li>
              ))}
            </ul>
          </div>
