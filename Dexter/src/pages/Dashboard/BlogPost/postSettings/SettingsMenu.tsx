...
const SettingsMenu = ({ selectedSetting, setSelectedSetting }: Props) => {
    ...
    <div className="relative ">
      <div className="max-md:hidden flex   flex-col  w-[16%]  md:w-[16%] ">
        {menuList.map((menu) => (
          <div
            key={menu.id}
            className={`text-sm ${
              menu.icon ? "flex gap-3 items-center" : "block"
            } ${
      ...
      <div className="w-[70%] p-3 absolute right-0   bg-white border rounded-lg lg:hidden md:hidden">
        <select
          ...
          onChange={(e) =>
            handleSelectSetting(e.target.value as unknown as number)
          }
        >
          {menuList.map((item) => (
