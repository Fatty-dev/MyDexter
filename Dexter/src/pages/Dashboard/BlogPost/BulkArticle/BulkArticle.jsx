  ...
  return (
    ...
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl sm:text-3xl font-semibold">Bulk Article Generation</h1>
    ...
    <div className="flex flex-col lg:flex-row justify-between gap-4 mb-4">
      <div className="lg:w-[80%] text-sm text-gray-600">
        <p>
          Add main keyword, Title, Keywords (optional) to create tasks for generating an article. Import from Excel if desired. Connect to Shopify, Wix, or WordPress to publish.
      ...
      <table className="w-full border-collapse text-sm">
        <thead className="whitespace-nowrap">
          <tr className="bg-gray-100 text-gray-700">
            {["Main Keyword", "Estimated Monthly Traffic", "Title", "Keywords"].map((header, idx) => (
              <th key={idx} className="py-3 px-4 text-left">
                <div className="flex items-center gap-2">
                  {header}
                  {(header === "Main Keyword" && (
                    <div onClick={generateMainKeyword} className="flex items-center gap-1 cursor-pointer hover:text-primary">
                      {loadingMainKeyword ? (
                    ...
                    (header === "Title" && (
                      <div onClick={generateTitle} className="flex items-center gap-1 cursor-pointer hover:text-primary">
                        {loadingTitle ? (
                          <span className="animate-spin">
                            <AiOutlineSync className="text-primary" size={16} />
                          </span>
                    ...
                    (header === "Keywords" && (
                      <div onClick={generateKeywords} className="flex items-center gap-1 cursor-pointer hover:text-primary">
                        {loadingKeywords ? (
                          <span className="animate-spin">
                            <AiOutlineSync className="text-primary" size={16} />
                          </span>
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={index} className="border-t border-gray-200">
              {["keyword", "estimatedMonthlyTraffic", "title", "keywords"].map((key, idx) => (
                <td key={idx} className="py-3 px-4">
                  <textarea
                    ...
                    rows={1}
                    value={key === "keywords" ? row[key].join(", ") : row[key]}
                    onChange={(e) => {
                      const value = key === "keywords" ? e.target.value.split(", ") : e.target.value;
