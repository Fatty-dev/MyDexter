...
const Personalization = () => {
  ...
  const customToggle = async (enabled) => {
    setLoading(true);
    try {
      const response = await authApi.put(`/settings/user/custom-instructions/toggle`, {
        enabled: enabled
      });

      if (response.data.success) {
        toast.success(`Custom instructions ${enabled ? "enabled" : "disabled"} successfully!`);
